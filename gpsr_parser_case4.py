...
start_words = ['tell', 'put', 'bring', 'count', 'follow','what',
				 ...
				 'introduce', 'escort', 'accompany','deliver','lead','pick','get','how','come','who']
...
def guide(person=[],obj=[],action=[],properties=[],location=[],query=[]):
	...
	tic = time.time()
	last_ask = time.time()
	ask_flag = 1 #no checking in gpsr guiding
	while time.time()-tic < 40 :
		...
		if time.time()-last_ask > 15 and ask_flag == 0 :
			pio.stop()
...
def find(person=[],obj=[],action=[],properties=[],location=[],query=[]): #TODO gender, objects? , add tags to find_target
	...
	if len(person)>0 :
		...
		if person[0] in male_names + female_names :
			fil=['waving']
			talk_ = person[0] + ', where are you? please raise your hand if you can!'
		else :
				...
				if len(fil)>0 and target_found.class_string == 'person' :
					if len(person)>0 :
						if person[0] in male_names+female_names :
							...
							pio.add_reid_target(200, fil[0], img=None, person_obj=target_found )
						else :
							name_of_person()
...
def name_of_person(person=[],obj=[],action=[],properties=[],location=[],query=[]):
	...
	while time.time()-tic1 < 60 :
		...
		pio.say('please come and tell me 1 second after beep')
		time.sleep(3)
...
def tell(person=[],obj=[],action=[],properties=[],location=[],query=[]):
	...
	if tell_content == 'nothing to tell' :
		...
		if 'time' in tell_content :
			now = datetime.datetime.now()
			nowstr = now.strftime("%H %M %B %d %Y")
			tell_content[ tell_content.index('time') ] = nowstr
		print tell_content
		pio.say(tell_content)
	return True
...
def convert(words):
	...
	while i < len(words) :
		...
		if words[i] in obj_words : obj.append(words[i])
...
pio.set_initial_pose_wp('entrance_outside',0.006,-0.006)
pio.map_clear_srv()
...
while cmd_cnt < 10 :
	...
	pio.speech_hints = all_words
	pio.say('say pepper. And give a command 1 second after beep')
	pio.speech_memory = ''
	cmd = ''
	got_cmd_flag = False
	...
	pio.say(cmd)
	pio.say('am i correct? please say pepper and say yes or no 1 second after beep')
	pio.speech_hints = ['yes','no']
